...
class SpeechTokenExtractor:
    def __init__(self, config_path: str, model_path: str, num_threads: int = 8, gpu_id: int = 0):
        ...
        # Properly configure the ONNX runtime to use the specified GPU
        ...
        # Get audio parameters from the config
    ...
    def process_audio(self, audio_path: str) -> tuple:
        ...
        try:
            ...
            # Extract features with whisper (running on GPU)
            ...
            # ONNX requires numpy input, so move back to CPU
    ...
    def extract_from_dataset(self, dataset_path: str, save_path: str):
        ...
        def process_and_save(audio_file):
            ...
            if audio_id:
                # Save a separate .pt file for each audio clip
