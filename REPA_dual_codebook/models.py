...
class AudioSetTokenizer(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, vocab_size):
        ...
        Args:
            ...
            hidden_dim (int): Dimension of the internal hidden layers, also the dimension of the codebook vectors.
    ...
    def forward(self, x):
        ...
        try:
            ...
            # Check for NaN or Inf to prevent training from being interrupted
    ...
    def tokenize(self, x):
        ...
        with torch.no_grad():
            ...
            # Handle the dimensionality of AST token indices, ensuring it is 2D (batch_size, seq_len)
...
class MelSpectrogramExtractor:
    ...
    def __init__(self, config, target_device=None):
        ...
        self.power = self.mel_config['power']  # Exponent of the magnitude spectrogram (e.g., 2.0 for the energy spectrogram)
    ...
    def __call__(self, waveform, normalize=True):
        """
        Use vectorized operations instead of a for loop for better efficiency and robustness.
        """
        ...
        # If required, normalize using the global statistics
        if normalize:
...
class DiTBlock(nn.Module):
    ...
    A DiT block typically contains a self-attention layer and a feed-forward network layer, both using residual connections and modulated before being fed into these layers.
    The modulation parameters are derived from the timestep embedding.
    """
    ...
    def __init__(self, dim, heads=16, ff_mult=4, dropout=0.2):
        ...
        Args:
            ...
            heads (int, optional): Number of heads in multi-head attention. Defaults to 16.
        ...
        # elementwise_affine=False means no learnable affine parameters (gamma, beta) are used, since these will be provided by the timestep modulation
        ...
        # Feed-forward network (typically a two-layer linear network with an activation function in between)
        ...
        # Time modulation network (Time MLP), used to generate modulation parameters (scale, shift, gate, etc.) from the timestep embedding
        ...
        self.time_mlp = nn.Sequential(
            nn.SiLU(),  # SiLU (Swish) activation function, applied to the input timestep embedding
            nn.Linear(dim, 6 * dim)  # Linear layer mapping the timestep embedding to 6x the dimension
        )
    ...
    def _init_weights(self):
        ...
        def _init_module_weights(m, gain=1.0, zero_out_last_linear=False):
            ...
            if isinstance(m, nn.Linear):
                if zero_out_last_linear:  # For the last linear layer of a residual block, initialize to 0
                    nn.init.zeros_(m.weight)
                else:
                    # Xavier uniform initialization with a gain parameter, helps preserve the signal variance
        ...
        _init_module_weights(self.ff[0])  # The first linear layer of the feed-forward network (expands the dimension)
        if hasattr(self.ff[3], 'weight'):  # The second linear layer of the feed-forward network (compresses back to dim, part of the residual path)
    ...
    def forward(self, x, t_emb):  # x: [B, T_seq, HiddenDim], t_emb: [B, HiddenDim]
        ...
        """
        # 1. Generate modulation parameters from the timestep embedding
...
class FlowMatchingModel(nn.Module):
    """
    Unified Flow Matching model (V-Simple - dual-token fusion):
      - Keeps dual-token fusion (S3/AS + BEATs interleaved 1:1).
      - Introduces REPA-style in-layer bypass alignment (dual teachers: Whisper/BEATs).
      - Uses only the Flow Matching loss + the REPA bypass distillation loss.
    """
    ...
    def __init__(self, config, teacher_dim_speech: int = 0, teacher_dim_audio: int = 0):
        ...
        # ===== Separate tables + linear projection to a shared space (dual-token fusion) =====
        ...
        with torch.no_grad():
            # Initialize the projection layers for the concatenation path
        ...
        self.max_fused_embed_dim = D_tok  # After fusion, project back to D_tok
        self.input_proj = nn.Linear(self.n_mels + self.max_fused_embed_dim + self.ref_mel_dim, hidden_dim)

        # ===== Timestep embedding + DiT backbone + output layer =====
        ...
        # ====== REPA layer-wise bypass: layer set & lambda (from FoG-A x2) ======
        # 1-based -> 0-based
        self.repa_layers_speech = [0, 2, 22]  # L1=0, L3=2, L23=22
        self.repa_layers_audio = [0, 2, 6]  # L1=0, L3=2, L7=6
        # lambda per layer
        self.repa_lambda_speech =  {0: 0.506, 2: 0.1308, 22: 0.1082}
        self.repa_lambda_audio = {0: 0.388, 2: 0.1026, 6: 0.0856}

        # A dedicated MLP head per layer: hidden_dim -> teacher_dim
        def _make_mlp_head(out_dim: int):
            # A concise two-layer MLP; REPA recommends using an MLP for h_phi(.); no activation at the end so it can be cosine-aligned with the teacher
    ...
    def _predict_velocity(
        ...
        Args:
            xt: [B, n_mels, T], the intermediate state constructed in FM by (1-t)*noise + t*x.
            ...
            capture_layers: (for the probe) if a set of layer indices (0-based) is given, only perform pooled capture for these layers.
            return_layer_pooled: (for the probe) when True, return a pooled dict {layer_idx: [B, hidden_dim]}.
    ...
    def embed_and_project_tokens(self, domain: str, token_indices: torch.LongTensor):
        ...
        - S3/AS tokens are at even positions, BEATs tokens are at odd positions.
        - Extract and embed them separately, then concatenate the embedding vectors, and finally project.
        - Only return the fused embedding.
        ...
        # Ensure the two sequences have the same length, in case the total length is odd
    ...
    # ===== Forward: FM main loss + REPA layer bypass distillation =====
    def forward(self, x, t, cond_embed_dict):
        """
        Compute the Flow Matching loss + REPA loss.
        x: [B, n_mels, T_full]
        t: [B]
        cond_embed_dict:
        ...
        if dom_ids is not None and (len(self.repa_heads_speech) > 0 or len(self.repa_heads_audio) > 0):
            ...
            for li, vec in pooled.items():  # vec: [B, hidden_dim]
                # Normalize (REPA's sim(.) recommends normalizing the representations for similarity)
            ...
            for li_key in self.repa_heads_speech:  # li_key is a str
                if li_key not in used_repa_speech_layers:
                    # This head was not used in this batch; add a dummy loss
    ...
    def sample(self, cond_embed_dict, *, target_duration_frames=None, steps=None, cfg_scale=None, method="euler",
        ...
        """
        Inference interface
        cond_embed_dict: dict containing:
