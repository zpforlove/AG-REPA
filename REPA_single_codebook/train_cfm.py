...
class BalancedDistributedSampler(DistributedSampler):
    ...
    By oversampling the smaller datasets, it ensures that within each epoch the number of samples provided by each subset
    equals the number of samples in the largest subset.
    """
    ...
    def __init__(self, dataset, num_replicas=None, rank=None, shuffle=True, seed=0, drop_last=False):
        ...
        # Correctly compute the number of samples per GPU and the aligned total number of samples based on the drop_last argument
        if self.drop_last:
    ...
    def __iter__(self):
        ...
        # --- Adjust the length of the index list based on the drop_last argument ---
...
class FlowMatchingTrainer:
    ...
    def _setup_external_models(self):
        ...
        - Load the BEATs Feature Extractor (Audio teacher + AST frontend).
        - Load Whisper-large-v3 (encoder only, Speech teacher).
        - Infer the teacher dimensions: self.teacher_dim_audio / self.teacher_dim_speech
        """
        try:
            # ====== AudioSetTokenizer (keeps the original logic) ======
            ...
            logger.info(f"Rank {self.cmd_args.local_rank}: BEATs Feature Extractor loaded.")

            # Infer the Audio teacher vector dimension (run a dummy pass)
            with torch.no_grad():
            ...
            try:
                ...
                # Read directly from config, or fall back to running a dummy pass to get the dimension
    ...
    def setup_models(self):
        ...
        try:
            ...
            # Pass in the teacher dimensions to ensure the projection heads are created before DeepSpeed initialization
    ...
    def _init_data_loaders(self):
        ...
        try:
            ...
            if is_dist:
                ...
                # In non-distributed mode, you could implement a non-distributed balanced sampler, or simply keep random shuffling
    ...
    def prepare_batch(self, batch):
        ...
        - The condition `fused_embed` comes directly from the respective S3 or AS tokens
        """
        try:
            device = self.device
            ...
            # -------------------- 3) Condition fused_embed (shared token space) --------------------
            ...
            if len(as_idx) > 0:
                ...
                with torch.no_grad():
                    ...
                    as_shared = self.model_engine.module.embed_and_project_tokens('as', as_tokens.to(device))
            ...
            if len(ls_idx) > 0:
                ...
                if s3_field is None:
                    raise RuntimeError("The batch is missing the 's3_token' field; please make sure LibriSpeechDataset correctly returns this key.")
                ls_tokens_list = []
                # ... (the s3_token handling logic remains unchanged)
                ...
                max_token_len = max(t.numel() for t in ls_tokens_list) if ls_tokens_list else 0
                if max_token_len == 0:
                    max_token_len = 1
                    ls_tokens_list = [torch.zeros(1, dtype=torch.long, device=device) for _ in ls_tokens_list]
                padded_tokens = torch.stack([
                    F.pad(t.to(device), (0, max_token_len - t.numel()), value=0) for t in ls_tokens_list
                ], dim=0)
    ...
    def train_step(self, batch):
        ...
        Single training step:
          1) Standard FM forward/backward
        """
    ...
    def save_checkpoint(self, tag: str, client_state: dict):
        # Ensure all processes synchronize before rank 0 begins saving
        if torch.distributed.is_initialized():
            torch.distributed.barrier()
