...
class LLMTrainer:
    def __init__(self, config_path, cmd_args):
        ...
        # ID mappings for [AS_k] and [AUDIO_i] in the tokenizer
        self.as_token_ids = []  # len = vocab_size, the tokenizer ids corresponding to [AS_0..AS_{V-1}]
        self.id_to_as_code = {}  # tokenizer id -> k (AS_k)
        self.audio_token_ids = []  # len = style_num_tokens, the tokenizer ids corresponding to [AUDIO_0..K-1]

        # Mapping from coarse codewords to cluster ids
    ...
    def _load_metadata(self):
        ...
        def load_segments_df(csv_path):
            try:
                ...
                print(f"CSV segment file loaded successfully: {csv_path}")
                return df
            except Exception as e:
                logger.error(f"Failed to load the CSV file: {csv_path}, error: {e}")
                raise

        # Load the training and validation segment data
    ...
    def _setup_model_and_deepspeed(self):
        ...
        if hidden_size is None:
            # Compatibility with the field names of some models
            hidden_size = getattr(model.config, "n_embd", None) or getattr(model.config, "dim", None)
            if hidden_size is None:
                raise ValueError("Could not infer hidden_size from the LLM configuration.")

        # 1) style projection layer: BEATs -> LLM hidden dimension
        model.style_proj = nn.Linear(self.beats_embed_dim, hidden_size)
        # Injection strength and Dropout probability tunable during training (saved on the model for distributed consistency)
    ...
    def _build_coarse_mapping(self):
        """Build the mapping from codewords -> coarse cluster ids"""
        ...
        if self.coarse_code_to_cluster_path and os.path.exists(self.coarse_code_to_cluster_path):
            try:
                with open(self.coarse_code_to_cluster_path, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                # May be a dict {"0":12,...} or a list[cluster_id]*V
    ...
    def _setup_dataloaders(self, is_train=True):
        ...
        if is_train:
            ...
            print(f"Training data loader initialization complete, containing {len(self.train_loader)} batches.")
        else:
            ...
            print(f"Validation data loader initialization complete, containing {len(self.val_loader)} batches.")
    ...
    def setup_lr_scheduler(self):
        ...
        if self.pytorch_optimizer is None or self.train_loader is None:
            logger.error(
                f"Rank {self.cmd_args.local_rank}: The optimizer or the training data loader is not initialized; cannot create the LR scheduler.")
            return
    ...
    def _extract_as_and_style(self, waveform, K):
        ...
        Extract from the audio waveform:
          - AST target tokens (as ground-truth)
          - K BEATs style chunks (evenly split in time) -> [K, D_beats]
        """
        ...
        # Compute the K style chunks: split evenly in time, then take the mean
    ...
    def _prepare_batch_data(self, batch, is_train):
        ...
        # Build labels: mask out the prompt and the pad
        ...
        if len(valid_indices) < B:
            logger.warning(f"Discarding {B - len(valid_indices)} invalid samples from the batch.")
            input_ids = input_ids[valid_indices]
            attention_mask = attention_mask[valid_indices]
            labels = labels[valid_indices]
            style_feats = style_feats[valid_indices]

        return input_ids, attention_mask, labels, style_feats  # Also returns style_feats
    ...
    def _inject_style_inputs_embeds(self, input_ids, style_feats, is_train: bool):
        ...
        where alpha can undergo Style Dropout during training (the CFG style-free path).
        """
        ...
        # Style Dropout: during training, set alpha=0 with probability p; not applied during validation
        p = self.model_engine.module.style_dropout_p if is_train else 0.0
        alpha_base = float(self.model_engine.module.style_strength)
        # Generate alpha for each sample
        alphas = []
        for b in range(B):
        ...
        for i in range(K):
            ...
            # Expand the i-th style chunk to [B,1,H], then broadcast to T
    ...
    def _compute_coarse_loss(self, hidden_states, labels):
        ...
        Compute the auxiliary loss of the coarse head:
          - Only at positions where labels != -100 and the label is [AS_k], supervise its coarse cluster id
        """
        # hidden_states: [B, T, H] (the last layer)
    ...
    def train(self):
        ...
        # Auxiliary loss weight (can be added in config as hyperparameters.llm.coarse_loss_weight, default 0.5)
        ...
        for epoch in range(num_epochs):
            ...
            for step, batch in enumerate(pbar):
                ...
                # Build the inputs_embeds with style injection
    ...
    def validate(self):
        ...
        with torch.no_grad():
            for batch in pbar:
                ...
                # No Style Dropout during validation (alpha=style_strength)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train an LLM on AudioSet with DeepSpeed (with style prefix & Coarse head)")
